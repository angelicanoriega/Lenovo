...
const paint = (imgModelo, modelo, html) => {
    const divColum = document.createElement('div');
    divColum.setAttribute('class', 'col-md-4');
    const divCard = document.createElement('div');
    divCard.setAttribute('class', 'card  d-flex justify-content-center align-items-center');
    ...
    a.addEventListener('click',()=>{
        ...
        firebase.database().ref("notebook").child(title.innerHTML).on("value", snap => {
...
const notebook = firebase.database().ref("notebook");
const img = 'https://github.com/OshinVillegas/Lenovo/blob/gh-pages/mi%20data/notebook/ideapad%20320/80XQ000DLM/02_Ideapad_320_14inch_Hero_Front_facing_right_Video_chatting.png?raw=true';
notebook.on("value", snap => {
    const key = Object.keys(snap.val());
    key.forEach(elementKey => {
        paint(img, elementKey, document.getElementById('tabla'));
    });
})
